...
- **Cluckers**: Native Linux CLI launcher for Realm Royale on the Project Crown private server
- **Language**: Go 1.25, single binary, Linux-only (amd64), runs game via Wine/Proton-GE
...
- **CI**: GitHub Actions (build, test, vet) on all branches
...
- `cluckers steam add` -- Create .desktop file for Steam library integration
...
- `steam.go`: `steam add` subcommand, creates `.desktop` file, detects Steam Deck for tailored instructions.

### `internal/config/`
Configuration and paths.
- `config.go`: `Config` struct (Gateway, WinePath, WinePrefix, GameDir, HostX, Verbose). Loaded via viper from `~/.cluckers/config/settings.toml` (optional). Precedence: CLI flag > config file > default.
- `paths.go`: `DataDir()` (respects `CLUCKERS_HOME` env), `ConfigDir()`, `CacheDir()`, `ConfigFile()`, `CredentialsFile()`, `EnsureDir()`.
...
- `cache.go`: `TokenCache` struct with `AccessToken`, `OIDCToken`, `Username`, `CachedAt`. TTLs: access=24h, OIDC=55min. Stored as JSON in `~/.cluckers/cache/tokens.json` (0600 perms).
...
- `download.go`: `DownloadGameZip()` (HTTP Range resume, progress bar, ~5.3GB), `VerifyBLAKE3()`, `DownloadAndVerify()` (download + verify, deletes corrupt), `checkDiskSpace()` (needs 2x zip size).
- `extract.go`: `ExtractZip()` (zip-slip protection, progress counter, removes zip after extraction).

### `internal/wine/`
Wine/Proton-GE detection and prefix management.
