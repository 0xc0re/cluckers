...
static void proxy_init(void) {
    ...
    if (logf) { fprintf(logf, "REMAP: No working backend found!\n"); fflush(logf); }
}
...
__declspec(dllexport) DWORD WINAPI XInputGetState(DWORD idx, void *state) {
    ...
    n++;
    if (logf && (n <= 30 || n % 2000 == 0)) {
        fprintf(logf, "GetState(%lu->%lu)=%lu [#%d]\n", idx, real_idx, r, n);
        fflush(logf);
    }
    return r;
}
...
BOOL WINAPI DllMain(HINSTANCE h, DWORD reason, LPVOID res) {
    ...
    if (reason == 0 && logf) { fprintf(logf, "REMAP: unloading after %d calls\n", n); fclose(logf); }
    return TRUE;
}
